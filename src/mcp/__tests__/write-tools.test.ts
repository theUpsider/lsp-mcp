...
class FakeRegistrar {
  ...
  public registerTool(name: string, _config: { description?: string }, handler: (args: Record<string, unknown>) => Promise<unknown>): void {
...
describe('registerWriteTools', () => {
  ...
  it('renames symbols when the server supports rename and saves the changed file', async () => {
    ...
    expect(writeFile).toHaveBeenCalledWith('/workspace/src/index.ts', 'const foo = newName\n', 'utf8');
  ...
  it('handles missing clients, formatter defaults, and empty code actions', async () => {
    ...
    const client = createClient([]);
    (access as jest.MockedFunction<typeof access>).mockRejectedValue(new Error('missing'));
...
function createClient(result: unknown, capabilities: Record<string, unknown> = { renameProvider: true }): MockClient {
  return {
