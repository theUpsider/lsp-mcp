...
describe("registerReadTools", () => {
  ...
  it("initializes LSP with a valid root and reports health", async () => {
    ...
    await expect(
      getHandler(registrar, "lsp_init")({ root: "/workspace" }),
    ).resolves.toEqual({
      content: [
        {
          type: "text",
  ...
  it("rejects invalid lsp_init roots clearly", async () => {
    ...
    (stat as jest.MockedFunction<typeof stat>).mockRejectedValueOnce(
      new Error("ENOENT"),
    );
    ...
    await expect(
      getHandler(registrar, "lsp_init")({ root: "/missing/project" }),
    ).resolves.toEqual({
      content: [
  ...
  it("rejects missing, relative, and non-directory lsp_init roots", async () => {
    ...
    await expect(
      getHandler(registrar, "lsp_init")({ root: "relative/path" }),
    ).resolves.toEqual({
      content: [
        {
          type: "text",
    ...
    await expect(
      getHandler(registrar, "lsp_init")({ root: "/workspace/file.ts" }),
    ).resolves.toEqual({
      content: [
        {
          type: "text",
  ...
  it("returns cached file diagnostics and aggregates workspace diagnostics", async () => {
    ...
    registerReadTools(
      registrar,
      createLifecycle({ diagnostics, workspaceClients: [createClient([])] }),
      { initializeManager: jest.fn() },
    );
  ...
  it("returns restart guidance when the LSP crashed", async () => {
    const registrar = new FakeRegistrar();
    const client = createClient(
      new Error("LSP server exited unexpectedly (code: 1, signal: null)"),
    );
    registerReadTools(registrar, createLifecycle({ fileClient: client }), {
      initializeManager: jest.fn(),
    });
  ...
  return {
    ...
    getFileDiagnostics: jest.fn((_: string) =>
      (options.diagnostics ?? []).filter(
        (diagnostic) => diagnostic.uri === "file:///workspace/src/index.ts",
      ),
    ),
    getWorkspaceDiagnostics: jest.fn(
      (_: string | undefined) => options.diagnostics ?? [],
    ),
